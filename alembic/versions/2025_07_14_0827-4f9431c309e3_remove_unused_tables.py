...
def downgrade() -> None:
    op.execute("create type search_result as enum ('found_results', 'no_results_found');")
...
def _create_search_results_table():
    op.create_table(
        ...
        sa.Column('search_id', sa.Integer, sa.ForeignKey('search_queue.search_id'), nullable=False),
...
def _create_search_queue_table():
    op.create_table(
        ...
        sa.Column('batch_id', sa.Integer, sa.ForeignKey('search_batch_info.batch_id'), nullable=False),
        sa.Column('search_query', sa.Text, nullable=False),
        sa.Column('executed_datetime', sa.TIMESTAMP),
        enum_column('status', enum_name=SEARCH_STATUS_ENUM_NAME, enum_values=['pending', 'completed', 'error', 'no_results']),
    )
...
def _create_search_batch_info_table():
    ...
    op.create_table(
        ...
        sa.Column('initiated_datetime', sa.TIMESTAMP, nullable=False, server_default=sa.text('now()'))
    )