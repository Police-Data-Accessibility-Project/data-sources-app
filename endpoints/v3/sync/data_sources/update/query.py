...
class SourceManagerUpdateDataSourcesQueryBuilder(QueryBuilderBase):
    ...
    def run(self) -> None:
        ...
        for data_source_request in self.request.data_sources:
            ...
            for key, value in data_source_request.model_dump(exclude_unset=True).items():
        ...
        self.bulk_update_mappings(
            DataSource, bulk_update_mappings,
        )
        ...
        for data_source_request in self.request.data_sources:
            if data_source_request.agency_ids is not None:
                ds_id_agency_id_mappings[data_source_request.app_id] = data_source_request.agency_ids
