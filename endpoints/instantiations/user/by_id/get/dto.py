...
class ExternalAccountDTO(BaseModel):
    github: Optional[str] = default_field_required(
        description="The GitHub user id of the user"
    )
...
class UserProfileResponseSchemaInnerDTO(BaseModel):
    ...
    recent_searches: GetUserRecentSearchesOuterDTO = default_field_required(
        description="The recent searches of the user"
    )
    followed_searches: GetUserFollowedSearchesDTO = default_field_required(
        description="The followed searches of the user"
    )
    data_requests: GetManyDataRequestsResponseDTO = default_field_required(
        description="The data requests of the user"
    )
    permissions: list[PermissionsEnum] = default_field_required(
