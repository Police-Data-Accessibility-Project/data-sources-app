...
def main():
    ...
    data_dumper_docker_info = DockerInfo(
        dockerfile_info=DockerfileInfo(
            image_tag="datadumper", dockerfile_directory="local_database/DataDumper"
        ),
    ...
    app_docker_info = DockerInfo(
        dockerfile_info=DockerfileInfo(
            image_tag="data_sources_app", dockerfile_directory=""
        ),
        name="data_sources_app_host",
        ports={"8000/tcp": 8000},
        environment={
            "DO_DATABASE_URL": get_from_env("DO_DATABASE_URL"),
            "GH_CLIENT_ID": get_from_env("GH_CLIENT_ID"),
            "GH_CLIENT_SECRET": get_from_env("GH_CLIENT_SECRET"),
            "FLASK_APP_COOKIE_ENCRYPTION_KEY": get_from_env(
