...
export default {
  ...
  methods: {
    async search() {
      const headers = {"Authorization": `Bearer ${process.env.local.VUE_APP_PDAP_TOKEN}`}
      const res = await axios.get(`${process.env.local.VUE_APP_BASE_URL}/quick-search/${this.searchTerm}/${this.location}`, {headers})
