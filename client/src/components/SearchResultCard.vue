<template>
	<GridItem
		...
		alignment="center"
		class="search-result-card small"
		data-test="search-result-card"
	>
		<h2 class="search-result-title" data-test="search-result-title">
...
-->
		...
		<p class="search-result-data" v-else data-test="search-result-record-type-unknown">
			Record Type Unknown
		</p>
		<p
			class="search-result-label"
		...
		>
			{{ formatDate(dataSource.coverage_start) }}-{{ formatDate(dataSource.coverage_end) }}
		</p>
		<p
			class="search-result-data"
		...
		<FlexContainer v-if="dataSource.record_format" data-test="search-result-formats">
			<p
				class="search-result-data"
				:key="recordFormat"
		...
		<p class="search-result-data" data-test="search-result-format-unknown" v-else>
...
.search-result-card {
	--tw-border-opacity: 1;
	border: 2px solid rgba(var(--color-neutral-950) / var(--tw-border-opacity));
	margin: 1rem;
}

.search-result-card.pdap-flex-container.pdap-grid-item {
	padding: 1rem;
}
