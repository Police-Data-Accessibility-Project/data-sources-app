                        ...
                        if (match[1]) {
                            ...
                            if (unnestableTagNameIndex !== -1) {
                                ...
                                while (currentNode !== root) {
                                    if (currentNode[PropertySymbol.tagName] === tagName) {
                                        stack.pop();
                            ...
                            // End tag.
                            if (match[2].toUpperCase() === currentNode[PropertySymbol.tagName]) {
                                // Some elements are not allowed to be nested (e.g. "<a><a></a></a>" is not allowed.).
                                // Therefore we need to auto-close the tag, so that it become valid (e.g. "<a></a><a></a>").
                                const unnestableTagNameIndex = unnestableTagNames.indexOf(currentNode[PropertySymbol.tagName]);
                                if (unnestableTagNameIndex !== -1) {
                                    unnestableTagNames.splice(unnestableTagNameIndex, 1);
                                }
                                stack.pop();
                    ...
                    case MarkupReadStateEnum.insideStartTag:
                        ...
                        if (match[7] || match[8]) {
                            ...
                            if (!!attributeString) {
                                ...
                                while ((attributeMatch = attributeRegexp.exec(attributeString))) {
                                    if ((attributeMatch[1] && attributeMatch[2]) ||
                                        ...
                                        const namespaceURI = currentNode[PropertySymbol.tagName] === 'SVG' && name === 'xmlns'
                                            ? value
                                            : null;
                            ...
                            if (hasAttributeStringEnded) {
                                ...
                                if (VoidElements_js_1.default[currentNode[PropertySymbol.tagName]] ||
                                    (match[7] &&
                                    ...
                                    plainTextTagName = PlainTextElements_js_1.default[currentNode[PropertySymbol.tagName]]
                                    ...
                                    if (UnnestableElements_js_1.default[currentNode[PropertySymbol.tagName]]) {
