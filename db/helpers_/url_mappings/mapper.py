...
class URLMapper:
    def __init__(self, mappings: list[URLMapping]):
        self._url_to_id = {mapping.url: mapping.url_id for mapping in mappings}
        self._id_to_url = {mapping.url_id: mapping.url for mapping in mappings}
    ...
    def get_ids(self, urls: list[str]) -> list[int]:
        return [self._url_to_id[url] for url in urls]
    ...
    def get_mappings_by_url(self, urls: list[str]) -> list[URLMapping]:
        return [URLMapping(url_id=self._url_to_id[url], url=url) for url in urls]
