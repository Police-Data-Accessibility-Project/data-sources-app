...
from middleware.dataclasses import GithubUserInfo, FlaskSessionCallbackInfo, OAuthCallbackInfo
...
def get_oauth_callback_info() -> OAuthCallbackInfo:
    ...
    return OAuthCallbackInfo(
        github_user_info=get_github_user_info(token)
    )


def callback_outer_wrapper(
...
def link_github_account_request(
    ...
    return make_response({"message": "Successfully linked Github account"}, HTTPStatus.OK)
