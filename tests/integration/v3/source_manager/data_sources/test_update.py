...
from db.enums import DetailLevel, AccessType, UpdateMethod, RetentionSchedule, URLStatus, AgencyAggregation
from db.models.implementations import LinkAgencyDataSource
from db.models.implementations.core.data_source.core import DataSource
from endpoints.v3.sync.data_sources.update.request import UpdateDataSourcesOuterRequest, UpdateDataSourcesInnerRequest
...
def test_data_source_manager_data_sources_update(
    ...
    api_test_helper.request_validator.post_v3(
        url=f"/source-manager/data-sources/update",
    ...
    assert data_source_1['agency_supplied'] == True
    ...
    assert data_source_1['retention_schedule'] == RetentionSchedule.LESS_THAN_ONE_DAY.value
    assert data_source_1['scraper_url'] == "https://www.example.com/scraper"
    assert data_source_1['agency_described_not_in_database'] == "Test agency described not in database"
    ...
    assert data_source_2['access_types'] == [AccessType.API.value, AccessType.DOWNLOAD.value]
    ...
    assert data_source_2['retention_schedule'] == RetentionSchedule.LESS_THAN_ONE_DAY.value
    assert data_source_2['scraper_url'] == "https://www.example.com/scraper"
    assert data_source_2['agency_described_not_in_database'] == "Updated agency described not in database"
