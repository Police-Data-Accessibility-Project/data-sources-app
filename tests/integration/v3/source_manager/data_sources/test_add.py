...
from endpoints.v3.sync.data_sources.add.request import AddDataSourcesOuterRequest, AddDataSourcesInnerRequest
...
def test_source_manager_data_sources_add(
    ...
    response: SourceManagerSyncAddOuterResponse = api_test_helper.request_validator.post_v3(
        url=f"/source-manager/data-sources/add",
    ...
    assert data_source_1['agency_originated'] == False
    assert data_source_1['agency_aggregation'] == AgencyAggregation.LOCAL.value
    assert data_source_1['agency_described_not_in_database'] == "Test described not in database"
    ...
    assert data_source_1['access_types'] == [AccessType.API.value, AccessType.DOWNLOAD.value]
    ...
    assert data_source_2['agency_supplied'] == True
    assert data_source_2['agency_originated'] == False
