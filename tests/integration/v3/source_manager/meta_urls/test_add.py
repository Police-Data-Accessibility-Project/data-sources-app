...
from endpoints.v3.sync.meta_urls.add.request import AddMetaURLsOuterRequest, AddMetaURLsInnerRequest
...
def test_source_manager_meta_urls_add(
    ...
    response: SourceManagerSyncAddOuterResponse = api_test_helper.request_validator.post_v3(
        url=f"/source-manager/meta-urls/add",
    ...
    assert {
        r.request_id for r in response.entities
    } == {
        1,
        2,
    }
