...
from middleware.third_party_interaction_logic.github_issue_api_logic import GithubIssueInfo, GithubIssueProjectInfo
from resources.endpoint_schema_config import SchemaConfigs
from tests.helper_scripts.common_test_data import TestDataCreatorFlask, get_random_number_for_testing
from tests.helper_scripts.constants import GITHUB_DATA_REQUESTS_ISSUES_ENDPOINT, DATA_REQUESTS_BY_ID_ENDPOINT, \
...
def generate_fake_github_issue_info() -> GithubIssueInfo:
    ...
    return GithubIssueInfo(
        url=f"https://github.com/cool-github-issue-url/{number}",
        number=number
    )
...
def test_github_data_requests_issues_post(test_data_creator_flask: TestDataCreatorFlask, monkeypatch):
    ...
    mock_create_github_issue = lambda *args, **kwargs: GithubIssueInfo(
        url=mock_github_issue_url,
        number=fake_github_issue_info.number
    )
    monkeypatch.setattr(
        f"{PATCH_ROOT}.create_github_issue",
...
def test_synchronize_github_issue(
    ...
    def add_fake_github_info_for_data_request(
            data_request_id: int
    ) -> GithubIssueInfo:
    ...
    gipi.add_project_status(issue_number=fgi_1.number, project_status=RequestStatus.ACTIVE.value)
    gipi.add_project_status(issue_number=fgi_2.number, project_status=RequestStatus.COMPLETE.value) # This one will yield a db update
    ...
    def mock_get_github_issue_project_statuses(issue_numbers: list[int]) -> GithubIssueProjectInfo:
        assert issue_numbers == [fgi_1.number, fgi_2.number], f"Unexpected issue number in {issue_numbers}; expected only {fgi_1.number} and {fgi_2.number}"
        return gipi

    # Patch `git_github_issue_project_statuses` to return the mock
    monkeypatch.setattr(f"{PATCH_ROOT}.get_github_issue_project_statuses", mock_get_github_issue_project_statuses)
