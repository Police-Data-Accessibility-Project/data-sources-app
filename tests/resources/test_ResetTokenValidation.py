...
def test_post_reset_token_validation(client_with_mock_db, monkeypatch):
    ...
    mock_reset_token_validation_result = (
        {"message": "Test Response"},
        HTTPStatus.IM_A_TEAPOT,
    )
    mock_reset_token_validation = MagicMock(
        return_value=mock_reset_token_validation_result
    )

    # Patch
    monkeypatch.setattr(
        "resources.ResetTokenValidation.reset_token_validation",
        mock_reset_token_validation,
    )

    # Call endpoint
    response = client_with_mock_db.client.post(
        "/reset-token-validation", json=mock_data
    )
