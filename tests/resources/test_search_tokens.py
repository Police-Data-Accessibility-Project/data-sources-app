...
def test_get_search_tokens(client_with_mock_db, monkeypatch):
    ...
    mock_perform_endpoint_logic_result = ({"message": "Test Response"}, HTTPStatus.IM_A_TEAPOT)
    mock_perform_endpoint_logic = MagicMock(return_value=mock_perform_endpoint_logic_result)

    # Patch
    monkeypatch.setattr("resources.SearchTokens.insert_access_token", mock_insert_access_token)
    monkeypatch.setattr("resources.SearchTokens.perform_endpoint_logic", mock_perform_endpoint_logic)
