...
def test_put_agencies(client_with_mock_db, monkeypatch):
    ...
    mock_update_archives_data = MagicMock(return_value=({"message": "Test Response"}, HTTPStatus.IM_A_TEAPOT))
    monkeypatch.setattr("resources.Archives.update_archives_data", mock_update_archives_data)
