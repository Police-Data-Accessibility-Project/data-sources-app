...
def test_post_request_reset_password(
    ...
    mock_request_reset_password_result = ({"message": "Test Response"}, HTTPStatus.IM_A_TEAPOT)
    mock_request_reset_password = MagicMock(return_value=mock_request_reset_password_result)

    # Patch
    monkeypatch.setattr("resources.RequestResetPassword.request", mock_request)
    monkeypatch.setattr("resources.RequestResetPassword.request_reset_password", mock_request_reset_password)
