...
class DynamicMagicMock:
    ...
    mock = UpdateArchivesDataMocks(patch_root, mocks_to_patch, return_values)
    ...
    - build_mocks(self, return_values: dict): Builds the MagicMock objects.
    ...
    def __init__(self, patch_root: str = "", mocks_to_patch: list = None, return_values=None):
        self._patch_root = patch_root
        self.__post_init__(mocks_to_patch, return_values)

    def __post_init__(self, mocks_to_patch: list = None, return_values=None) -> None:
        self._patchers = {}
        if return_values is None:
            return_values = {}
        if mocks_to_patch:
            self.build_mocks_with_dynamic_patching(mocks_to_patch, return_values)
        else:
            self.build_mocks(return_values)

    def build_mocks(self, return_values: dict):
    ...
    def build_mocks_with_dynamic_patching(self, mocks_to_patch: list, return_values: dict):
        for attribute, attr_type in self.__annotations__.items():
            if attribute in mocks_to_patch:
